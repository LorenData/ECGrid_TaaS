...
def main(inn, out):
    ...
    for line in inn.getloop({'BOTSID': 'HEA'}):
        if not headerwritten:
            ...
            order_date = line.get({'BOTSID':'HEA','ORDERDATE':None})
            ...
            # DTM
            deldate = line.get({'BOTSID': 'HEA', 'DELIVERYDATE': None})
            notbeforedate = line.get({'BOTSID': 'HEA', 'NODELBEFORE': None})
            notafterdate = line.get({'BOTSID': 'HEA', 'NODELAFTER': None})
